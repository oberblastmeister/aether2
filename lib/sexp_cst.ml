...
let take_while_from ~f ~pos s =
  let rec go pos =
    ...
    else if f s.[pos] then go (pos + 1)
    else pos
  in
  go pos

let is_control_char c =
  Char.is_whitespace c || Char.equal c '(' || Char.equal c ')'
...
let tokenize s =
  let rec go pos =
    if pos >= String.length s then []
      ...
      match c with
      | '(' ->
          ({ span = Span.single pos; token = Token.LParen } : SpannedToken.t)
          :: go (pos + 1)
      | ')' -> { span = Span.single pos; token = Token.RParen } :: go (pos + 1)
      | c when Char.is_whitespace c -> go (pos + 1)
      | _ ->
          let pos' =
            take_while_from ~pos ~f:(fun c -> not (is_control_char c)) s
          in
          {
            span = { start = pos; stop = pos' };
            token = Atom (String.slice s pos pos');
          }
          :: go pos'
  in
  go 0
...
let parse_tokens ts =
  ...
  | [] -> Ok xs
  | _ ->
      Or_error.error_s [%sexp "unexpected tokens", (ts : SpannedToken.t list)]
...
let span = function Atom { span; _ } -> span | List { span; _ } -> span
