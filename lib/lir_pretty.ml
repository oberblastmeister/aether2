...
let pretty_instr_control i =
  ...
  | InstrControl.CondJump (v, j1, j2) ->
    Pretty.(
      List
        [ Atom "cond_jump"; pretty_value v; pretty_block_call j1; pretty_block_call j2 ])
  | InstrControl.Ret v ->
...
let pretty_block (label : Label.t) (block : Block.t) =
  Pretty.(
    ...
    @@ List.concat
         ...
         ; List.map ~f:pretty_assign block.body |> List_ext.end_with ~sep:(Ann Line)
  ...
  Pretty.(
    List.concat
      ...
      ; (if not ([%equal: Label.t] graph.entry graph.exit)
         then [ Ann Line; pretty_block graph.exit (Map.find_exn graph.blocks graph.exit) ]
         else [])
      ])
;;
...
let pretty_program (prog : Program.t) =
  Pretty.(
    list
    @@ List.concat
         [ List.map ~f:pretty_function prog.functions |> List.intersperse ~sep:(Ann Line)
         ])
;;
